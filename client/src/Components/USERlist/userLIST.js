...
const UserLIST = () => {
  ...
  function handlerEdit(e) {
    ...
    axios
      ...
      .then((res) => {
        var prod = allUsers.findIndex(
          (user) => user.id === res.data.data.id
        );
  ...
  function handleAdmin(e) {
    axios.post('http://localhost:4000/auth/promote', { id: selectUser.id, isAdmin: e.target.checked }).then((res) => {
